...
Fixed effects included time (month), treatment group, age, and ITN use, with a key treatment × time interaction to assess differential trends between groups. Model selection was based on likelihood ratio tests and AIC, with the random intercept + random slope model with interaction identified as the best-fitting model.
